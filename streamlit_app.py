...
with st.form("sample_form"):
    ...
    with col1:
        ...
        geo_context = st.text_area("Geography/Context (where sample was purchased)", key=" 6 ")
        Relationship_prov = st.text_area("Relationship with provider", key="  7")
        Sold_as = st.text_input("Sold as", key=" 8 ")
        alias = st.text_input("Alias ", key=" 9")
        used_prior = st.text_input("Sample used prior to test", key=" 10")
        Sample_form = st.text_input("Sample form", key="11 ")
    ...
    with col2:
        ...
        unit_price = st.number_input("Price per unit")
        unit_currency = st.text_input("Price currency", key=" 18")
        Gender = st.selectbox("Gender", options=["M", "F", "NB"], key=" 19")
        Age = st.number_input("Age", key=" 20")
        test_method = st.selectbox("Test method", options=test_list, key=" 21")
        service_type = st.text_input("Service type", key=" 22")
